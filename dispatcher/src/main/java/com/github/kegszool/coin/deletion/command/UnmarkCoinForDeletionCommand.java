package com.github.kegszool.coin.deletion.command;

import com.github.kegszool.command.callback.CallbackCommand;
import com.github.kegszool.coin.deletion.handler.CoinDeletionInteractionHandler;
import org.telegram.telegrambots.meta.api.objects.CallbackQuery;
import org.telegram.telegrambots.meta.api.methods.botapimethods.PartialBotApiMethod;

import org.springframework.stereotype.Component;
import org.springframework.beans.factory.annotation.Value;
import org.springframework.beans.factory.annotation.Autowired;

@Component
public class UnmarkCoinForDeletionCommand extends CallbackCommand {

    @Value("${menu.coin_deletion_menu.prefix.unselected_coin_prefix}")
    private String UNSELECTED_DELETION_COIN_PREFIX;

    @Value("${menu.coin_deletion_menu.name}")
    private String COIN_DELETION_MENU_NAME;

    private final CoinDeletionInteractionHandler coinDeletionInteractionHandler;

    @Autowired
    public UnmarkCoinForDeletionCommand(CoinDeletionInteractionHandler coinDeletionInteractionHandler) {
        this.coinDeletionInteractionHandler = coinDeletionInteractionHandler;
    }

    @Override
    protected PartialBotApiMethod<?> handleCommand(CallbackQuery callback) {
        return coinDeletionInteractionHandler.handleSelection(callback, COIN_DELETION_MENU_NAME);
    }

    @Override
    protected boolean canHandleCommand(String command) {
        return command.startsWith(UNSELECTED_DELETION_COIN_PREFIX);
    }
}